...
const Sidebar = ({ showSidebar, setShowSidebar, handleTouchStart, handleTouchEnd }) => {
  ...
  return (
    ...
    >
      <div
        className={`fixed left-0 top-0 h-full bg-gray-800 text-white text-lg transition-transform duration-300 ease-in-out transform w-64 z-50 ${showSidebar ? "translate-x-0" : "-translate-x-full"}`}
      >
        <div className="p-4 pt-20">
          <div className="space-y-4">
            ...
            {isSignedin && <button onClick={() => { navigate(`/${username}/search-tracks`), setShowSidebar(false) }} className="px-4 py-2 bg-transparent text-white rounded-lg w-full cursor-pointer"> Search Tracks </button>}
          </div>
        </div>
      </div>
      <button onClick={() => setShowSidebar(!showSidebar)} className="fixed right-0 top-1 bg-blue-500 text-white p-4 rounded-l-full z-50 cursor-pointer">
